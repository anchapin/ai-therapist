...
class TestVoiceCommandProcessor:
    ...
    def test_extract_emergency_parameters(self, processor):
        ...
        assert len(params["emergency_keywords"]) > 0
    ...
    def test_extract_meditation_parameters(self, processor):
        ...
        assert params["duration"] == "short"
        assert params["type"] == "breathing"
    ...
    async def test_match_command(self, processor):
        ...
        result = await processor._match_command("start a new session")
        
        assert result is not None
        assert result.command.name == "start_session"
        assert result.confidence > 0.0
        assert result.matched_text in ["start a new session", "start new session"]
    ...
    async def test_match_command_with_priority(self, processor):
        ...
        result = await processor._match_command("help")
        
        assert result is not None
        assert result.command.name == "high_priority"  # Should match higher priority
    ...
    async def test_match_command_with_cooldown(self, processor):
        ...
        result1 = await processor._match_command("cooldown test")
        assert result1 is not None
        assert result1.command.name == "cooldown_test"
        
        # Immediate second call should not match due to cooldown
        result2 = await processor._match_command("cooldown test")
        ...
        result3 = await processor._match_command("cooldown test")
